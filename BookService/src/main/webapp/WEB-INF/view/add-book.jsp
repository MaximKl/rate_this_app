...
<!DOCTYPE html>
    <head>
        <meta charset="utf-8">
        <title>Додайте або оновіть книгу</title>
    ...
    <body style="margin:0px; font-family:verdana;" onload="load()">
        ...
        <form:form id="send" style="height:100%;" modelAttribute="book" method="POST">
            <div style="display:flex; width:100%; min-height:100%;">
                <div style="width:50%; background-color:#f0c654; min-height:100%;">
                    <h2 style="margin-left:40px; margin-top:20px;">ТУТ ВИ МОЖЕТЕ ДОДАТИ АБО ОНОВИТИ ІНФОРМАЦІЮ ПРО КНИГУ</h2>
                    <div id="block" style="margin-left:30px;">
                        <div id="nameBlock" style="margin-top:40px;">
                            <label for="tags">Назва книги: </label>
                            <br>
                            <form:input style="width:300px;" id="bookTags" path="name" type="text" required='true' placeholder="Укажіть назву"/>
                        </div>
                        ...
                        <div id="releaseBlock">
                            <label for="tags">Дата перошої публікації</label>
                            <br>
                            <form:input id="bDate" type="date" path="releaseDate" required='true' placeholder="Укажіть дату публікації"/>
                        </div>
                        ...
                        <div id="lengthBlock">
                            <label for="tags">Розмір книги (у сторінках): </label>
                            <br>
                            <form:input type="text" path="size" required='true' placeholder="Укажіть розмір книги"/>
                        </div>
                        ...
                        <div id="descriptionBlock">
                            <label for="tags">Опис книги: </label>
                            <br>
                            <form:textarea type="text" path="description" required='true' placeholder="Опис книги" style="width:400px; height:100px;"/>
                        </div>
                        ...
                        <br>
                        <div id="authorBlock">
                            <label for="tags">Додайте нового автора, або оберіть зі списку праворуч: </label>
                            <br>
                            <input id="authorAdd" class="toBlock" type="button" value="Додати нового автора" onclick="addAuthor()"/>
                        </div>
                        <br>
                        <br>
                    </div>
                ...
                <div style="width:50%; background-color:#55aeed; min-height:100%;">
                    <a style="user-select:none; font-size:120px; cursor:pointer; color:#505250; right:0; margin-right:400px; position:absolute; text-decoration: none;" href="http://localhost:9191/books">&larr;</a>

                        <div class="checkbox-dropdown">
                          Оберіть країни
                          <ul class="checkbox-dropdown-list">
                          <c:forEach var="BCountry" items="${allCountries}">
                            <li>
                              <label> <form:checkbox path="countries" value="${BCountry}"/> ${BCountry.name} </label>
                            </li>
                          </c:forEach>
                          </ul>
                        </div>
                        ...
                        <div class="checkbox-dropdown">
                          Оберіть категорії
                          <ul class="checkbox-dropdown-list">
                          <c:forEach var="BCategory" items="${allCategories}">
                            <li>
                              <label> <form:checkbox path="categories" value="${BCategory}"/> ${BCategory.name} </label>
                            </li>
                            </c:forEach>
                          </ul>
                        </div>
                        ...
                        <div class="checkbox-dropdown">
                          Оберіть авторів
                          <ul class="checkbox-dropdown-list">
                          <c:forEach var="BAuthor" items="${allAuthors}">
                            <li>
                              <label> <form:checkbox path="authors" value="${BAuthor}"/>${BAuthor.name}&nbsp${BAuthor.surname}</label>
                            </li>
                            </c:forEach>
                          </ul>
                        </div>
    ...
    <script>
          ...
          $( function() {
            ...
            $( "#bookTags" ).autocomplete({
              source: availableTags
            });
          } );

            function load(){
              if(window.location.href=="http://localhost:9005/books/add"){
              ...
              function getCookie(cname) {
                ...
                function addAuthor(){
                    var newInput = document.createElement("INPUT");
                    newInput.setAttribute("class","toBlockAuthors");
                    newInput.setAttribute("type","text");
                    newInput.setAttribute("placeholder","Спочатку ім'я, потім прізвище актора");
                    newInput.setAttribute("style","width:300px;");
                    document.getElementById("authorBlock").appendChild(document.createElement("br"));
                    document.getElementById("authorBlock").appendChild(newInput);
                    document.getElementById("authorBlock").appendChild(document.createElement("br"));
                }
                ...
                function apply(){
                    ...
                    document.getElementById("authorAdd").disabled=true;
                    ...
                    const arrAuthors = document.getElementsByClassName("toBlockAuthors");
                    var authorsLink="";
                    for (let i = 0; i < arrAuthors.length; i++) {
                      arrAuthors[i].disabled = true;
                      authorsLink = authorsLink + arrAuthors[i].value + ";";
                    }

                    document.getElementById("send").setAttribute("action","http://localhost:9191/books/add?auths="+authorsLink+"&cats="+catsLink);
                }
                ...
                function addChange(){
                    ...
                    document.getElementById("authorAdd").disabled=false;
