...
<!DOCTYPE html>
        ...
        <div id="main">
                <div style="display: flex; width:100%; height:100px; background-color:#027a32; text-align:center; position:fixed; top:0px; z-index:1;">
                    <div style="width:60%; margin-top:24px; text-align:center;">
                        <h1 style="display:inline; margin-top:200px; font-size:40px; color:white;">Усі ігри</h1>
                    </div>
                    <div style="text-align:center; width:40%;">
                        ...
                        <input id="profile"
                               ...
                               type="button" value="Додати гру" onclick="addGame()"/>
                        <input id="exitB"
                               style="width:200px; height:40px; font-size:18px; margin-left:10px; margin-top:30px; cursor:pointer; display:none;"
            ...
            <div>
                ...
                <input id="gameTags" type="text" value="${searched}" style="width:600px; font-size:16px; margin-top:120px;height:30px; margin-left:300px; display:inline" placeholder="пошук гри по назві">
                ...
                <input type="button" style="width:150px; height:30px; margin-top:120px; margin-left:100px; display:inline; background-color:#f76579; cursor:pointer;" value="Відмінити пошук" onclick="window.location.href = 'http://localhost:9191/games'">
                <h3 style="text-align:center;">${mes}</h3>
            </div>


            <c:forEach var="game" items="${allGames}">
            <div id="${game.id}fullGame">
                <c:url var="toGame" value="/games/${game.id}"></c:url>
                <div style="background-color:#a2cef5;">
                <h1 style="text-align:center"><a style="color:#505250;" href="http://localhost:9191/games/${game.id}">${game.name}</a></h1>
                     <table class="bigT"style="margin:auto; max-width:85%; min-width:80%;">
                         ...
                         <tr>
                            ...
                            <td style:"max-width:300px;">
                                Дата виходу: <p class="bold"><fmt:formatDate value="${game.releaseDate}" pattern="dd.MM.yyyy" /></p>
                                <br>
                                <br>
                                Видавець: <p class="bold">${game.publisher.name}</p>
                                <br>
                                <br>
                                Розробники:
                                ...
                                <br>
                                Категорії:
                                <c:forEach var="cat" items="${game.categories}">
                                    <br>
                             ...
                             <td style="font-weight:bold; text-align:center; max-width:200px;">
                                 Загальна оцінка: ${game.commonMark}/100
                                 <br>
                                 <br>
                                 Ефекти: ${game.effectMark}/100
                                 <br>
                                 <br>
                                 Історія: ${game.storyMark}/100
                                 <br>
                                 <br>
                                 Геймплей: ${game.gameplayMark}/100
                                 <br>
                                 <br>
                                 Ідея: ${game.ideaMark}/100
                                 <br>
                                 <br>
                                 Звук: ${game.soundMark}/100
                             </td>
                             ...
                             <td style="border-right:0px;  border-bottom:0px;"><input style="width:200px; height:40px; font-size:18px; cursor:pointer;"
                                      type="button" value="Оцінити" onclick="window.location.href = '${toGame}'"/></td>

                              <td style="max-width:100px; min-width:0px;" ><input data-game-id="${game.id}" class="delB" type="button" style="display:none; width:150px; height:30px; background-color:#f76579; cursor:pointer;"
                                                       value="Видалити гру" onclick="deleteGame(this)"></td>
                         </tr>
                     </table>
                 </div>
                 <br>
    ...
    <script>
        ...
        function load(){
          if(window.location.href=="http://localhost:9001/games"){
              window.location.replace("http://localhost:9191/games");
          }
          ...
          if (nick =="" || id == "") {
            ...
            document.getElementById("regB").style.display = "none";
                if(admin!=""){
                    document.getElementById("addGameB").style.display = "inline";
       ...
       function exit() {
        ...
        function filtration(){
            window.location.replace("http://localhost:9191/games?gname="+$("#gameTags").val());
         }
         ...
         function acceptRemove(){
             $.ajax({
                 ...
                 data: "gameId="+gameToDelete.getAttribute("data-game-id"),
                 success: function(response){
                  if(response=="BAD"){
                      return;
                     }
                  if(response=="OK"){
                      document.getElementById(gameToDelete.getAttribute("data-game-id")+"fullGame").style.display="none";
                      document.getElementById("removeAccept").style.display="none";
                      document.getElementById('main').style="opacity:1";
                  }
                 },
