...
<!DOCTYPE html>
    <head>
        <meta charset="utf-8">
        <title>Сторінка гри - ${game.name}</title>
        ...
        <div id="main">
                <div style="display: flex; width:100%; height:100px; background-color:#027a32; text-align:center; position:fixed; top:0px; z-index:1;">
                    <div style="width:60%; margin-top:24px; text-align:center;">
                        <h1 style="display:inline; margin-top:200px; font-size:40px; color:white;">Сторінка гри - ${game.name}</h1>
                    </div>
            ...
            <a style="user-select:none; font-size:120px; cursor:pointer; color:#505250; left:0; top:0; margin-left:100px; margin-top:80px; position:absolute; text-decoration: none;" href="http://localhost:9191/games">&larr;</a>
            ...
            <div style="display:flex; width:100%; height:100%;">
                <div style="width:40%; margin-left:50px;">
                    <div style="margin-top:120px; padding-top:20px; background-color:#f7cc54; border-radius:30px;">
                        <h3 style="text-align:center;">Інформація про гру</h3>
                        <table>
                            <tr>
                                <td class="names">Назва:</td>
                                <td class="gInfo">${game.name}</td>
                            </tr>
                            <tr>
                                <td class="names">Розробник:</td>
                                <td class="gInfo">
                                <c:forEach var="dev" items="${game.developers}">
                                    ${dev.name},&nbsp;
                                </c:forEach>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="gInfo">${game.publisher.name}</td>
                            </tr>
                            <tr>
                                <td class="names">Бюджет:</td>
                                <td class="gInfo">${game.cost} $</td>
                            </tr>
                            <tr>
                                <td class="names">Дата релізу:</td>
                                <td class="gInfo"> <fmt:formatDate value="${game.releaseDate}" pattern="dd.MM.yyyy" /></td>
                            </tr>
                            <tr>
                                <td class="names">Країна створення:</td>
                                 <td class="gInfo">
                                <c:forEach var="country" items="${game.countries}">
                                    ${country.name},&nbsp;
                                </c:forEach>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td class="names" style="max-width:150px;">Загальний час, витрачений гравцями:</td>
                                <td class="gInfo">${game.spentTime} годин</td>
                            </tr>
                            <tr>
                                <td class="names">Опис:</td>
                                <td class="gInfo">${game.description}</td>
                            </tr>
                        </table>
                        <br>
                    </div>
                ...
                <div style="width:50%;">
                     <div id="scores" style="margin-top:120px; margin-right:50px; background-color:#5c97f7; border-radius:30px; display:none;">
                        ...
                        <table>
                            ...
                            <tr>
                                <td class="names" style="font-weight:bold;">Ефекти:</td>
                                <td style="font-weight:bold;">
                                <select style="width:190px; height:30px; text-align:center;" id="effectMark">
                                  <c:forEach var="n" items="${numbers}">
                                      <option value="${n}">${n}</option>
                                  </c:forEach>
                                  <option value="-1" selected label="--не обрано--"></option>
                            ...
                            <tr>
                                <td class="names" style="font-weight:bold;">Звук:</td>
                                <td style="font-weight:bold;">
                                <select style="width:190px; height:30px; text-align:center;" id="soundMark">
                                  <c:forEach var="n" items="${numbers}">
                                      <option value="${n}">${n}</option>
                                  </c:forEach>
                                  <option value="-1" selected label="--не обрано--"></option>
                            ...
                            <tr>
                                <td class="names" style="font-weight:bold;">Геймплей:</td>
                                <td style="font-weight:bold;">
                                <select style="width:190px; height:30px; text-align:center;" id="gameplayMark">
                                  <c:forEach var="n" items="${numbers}">
                                      <option value="${n}">${n}</option>
                                  </c:forEach>
                                  <option value="-1" selected label="--не обрано--"></option>
                            ...
                            <tr>
                                <td class="names" style="font-weight:bold;">Ідея гри:</td>
                                <td style="font-weight:bold;">
                                <select style="width:190px; height:30px; text-align:center;" id="ideaMark">
                                  <c:forEach var="n" items="${numbers}">
                                      <option value="${n}">${n}</option>
                                  </c:forEach>
                                  <option value="-1" selected label="--не обрано--"></option>
                            ...
                            <tr>
                                <td class="names" style="font-weight:bold;">Витрачений час (у годинах):</td>
                                <td style="font-weight:bold;">
                                    <input min="1" style="width:190px; height:30px; text-align:center;" id="spentTime" type="number" required='true' placeholder="Укажіть затрчений час"/>
                                </td>
                            </tr>
                        </table>
                        <br>
                      ...
                      <div id="regMessage" style="margin-top:120px; margin-right:50px; background-color:#5c97f7; border-radius:30px; display:none;">
                        <h2 style="text-align:center; padding:15px;">ЩОБ ОЦІНИТИ ГРУ, ПОТРІБНО ЗАРЕЄСТРУВАТИСЯ АБО УВІЙТИ!</h2>
                        <input id="regB"
                      ...
                      <div id="alreadyMessage" style="margin-top:120px; margin-right:50px; background-color:#5c97f7; border-radius:30px; display:none;" >
                        <h2 style="text-align:center; padding:15px;">ВИ ВЖЕ ОЦІНИЛИ ЦЮ ГРУ!</h2>
                        <h4 style="text-align:center; color:#2b1515; ">${wrong}</h4>
                        <input id="changeB"
                    ...
                    <div style="margin-top:30px; margin-right:50px; background-color:#4ff7ba; border-radius:30px;">
                        <h3 style="text-align:center; padding-top:30px; margin-top:20px;">Загально-середні оцінки на сайті</h3>
                            <table>
                                <tr>
                                    <td class="names" style="font-weight:bold;">Загальна оцінка:</td>
                                    <td style="font-weight:bold;">${game.commonMark}/100</td>
                                </tr>
                                <tr>
                                    <td class="names">Ефекти:</td>
                                    <td>${game.effectMark}/100</td>
                                </tr>
                                <tr>
                                    <td class="names">Історія:</td>
                                    <td>${game.storyMark}/100</td>
                                </tr>
                                <tr>
                                    <td class="names">Геймплей:</td>
                                    <td>${game.gameplayMark}/100</td>
                                </tr>
                                <tr>
                                    <td class="names">Ідея:</td>
                                    <td>${game.ideaMark}/100</td>
                                </tr>
                                <tr>
                                    <td class="names">Звук:</td>
                                    <td>${game.soundMark}/100</td>
                                </tr>
                            </table>
                           <br>
                           <br>
            ...
            <br>
                <div style="margin-top:40px;">
                    ...
                    <div id="userComs" style="margin-top:40px; display:none;">
                        ...
                        <c:forEach var="coms1" items="${thisGameComment}">
                    ...
                    <div style="margin-top:40px;">
                        ...
                        <c:forEach var="coms2" items="${gameComments}">
    ...
    <script>
        ...
        function load(){
          if(window.location.href=="http://localhost:9001/games/${id}"){
              window.location.replace("http://localhost:9191/games/${id}");
          }
       ...
       function exit() {
         ...
         function addScore(){
            ...
            var comMark = buffer.options[buffer.selectedIndex].value;
                ...
                buffer = document.getElementById('effectMark');
            var effectMark = buffer.options[buffer.selectedIndex].value;

                buffer = document.getElementById('storyMark');
            var storyMark = buffer.options[buffer.selectedIndex].value;

                buffer = document.getElementById('soundMark');
            var soundMark = buffer.options[buffer.selectedIndex].value;

                buffer = document.getElementById('gameplayMark');
            var gameplayMark = buffer.options[buffer.selectedIndex].value;

                buffer = document.getElementById('ideaMark');
            var ideaMark = buffer.options[buffer.selectedIndex].value;

            var spentTime = document.getElementById('spentTime').value;

            window.location.replace("http://localhost:9191/games/${id}?comM="+comMark+"&efM="+effectMark+"&stM="+storyMark+"&soundM="+soundMark+"&gamM="+gameplayMark+"&ideaM="+ideaMark+"&spt="+spentTime+"&toChange="+toChangeMarks);
         }
         ...
         function sendComment(){
             $.ajax({
                 type: "POST",
                 url: "http://localhost:9191/games/${id}",
